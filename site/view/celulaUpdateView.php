...
if (isset($_GET['id'])) {
    ...
    switch ($logtipo) {
        case 1:
            ...
            break;
        case 2:
            $cel->coordenadorReadWholebyId($id);
            $ret = $cel->getDadosWhole();
            break;
        case 3:
            ...
            break;
        case 4:
            $cel->liderReadWholebyId($id);
            $ret = $cel->getDadosWhole();
            break;
        case 5:
            $cel->colaboradorReadWholebyId($id);
            $ret = $cel->getDadosWhole();
            break;
        case 6:
            $cel->comumReadWholebyId($id);
            $ret = $cel->getDadosWhole();
            break;
        case 7:
            '0';
...
if (isset($_POST['celCelula'])) {
    ...
    switch ($logtipo) {
        case 1:
            $celup->updateCelulaforId($id);
            break;
        case 2:
            $celup->coordenadorUpdateCelulaforId($id);
            break;
        case 3:
            $celup->pastorUpdateCelulaforId($celId);
            break;
        case 4:
            $celup->liderUpdateCelulaforId($id);
            break;
        case 5:
            $celup->colaboradorUpdateCelulaforId($id);
            break;
        case 6:
            $celup->comumUpdateCelulaforId($id);
            break;
        case 7:
            '0';
            break;
...
<div class="container-fluid pb-5 mb-3">
	<div>
		<div class="mt-2">
			<a href="index.php?pag=site_view_celulasReadView"
				id="voltar"
				class=" text-primary" ><span
				class="d-inline-block "></span> voltar</a>
		<?php if($_SESSION['tipo']<= '3'){ ?>
			<a href="" data-toggle="modal" data-target="#modal-apaga"
				style="float: right;"> <span
				class="glyphicon glyphicon-trash btn-apaga"> <label
					class="acessoSmart" style='display: none'>Apagar</label>
			</span>
			</a>
		<?php } ?>	
		</div>
		<form action="" method="post" accept-charset="UTF-8">
			<div>
				<div class="mt-2">
					<h2 class="text-muted">Atualizar Registro da Célula   &nbsp;&nbsp;&nbsp;<?php echo strtoupper(@$ret[0]['celCelula']); ?></h2>
				</div>
				<div>
					<div class="form-row" style="margin-top: -10px;">
						<div id="id" class="">
							<input type="hidden" name="celId" id="id"
								value="<?php echo @$ret[0]['celId'];?>" class="">
						</div>
						<div id="celula" class="form-group col-md-6"
							style="margin-top: 30px; margin-bottom: 0px; margin-left: 0%;">
							<label for="celula">Célula</label> <input type="text"
								name="celCelula" id="celula"
								value="<?php echo strtoupper(@$ret[0]['celCelula']); ?>"
								onkeyup="maiuscula(this)" class="form-control">
						</div>
						<div id="rede" class="form-group col-md-6"
							style="margin-top: 30px; margin-bottom: 0px; margin-left: 0%;">
							<label for="rede">Rede</label> <input type="text" name="celRede"
								id="rede" value="<?php echo strtoupper(@$ret[0]['celRede']); ?>"
								onkeyup="maiuscula(this)" class="form-control">
						</div>
						<div id="lider" class="form-group col-md-6"
							style="margin-top: 2px;">
							<label for="lider">Lider</label> <input type="text"
								name="celLider" id="lider"
								value="<?php echo strtoupper(@$ret[0]['celLider']);?>"
								onkeyup="maiuscula(this)" class="form-control">
						</div>
					</div>
					<div class="form-row">
						<div id="vicelider" class="form-group col-md-6"
							style="margin-top: 2px;">
							<label for="vicelider">Vice-lider</label> <input type="text"
								name="celViceLider" id="vicelider"
								value="<?php echo strtoupper(@$ret[0]['celViceLider']);?>"
								onkeyup="maiuscula(this)" class="form-control">
						</div>
						<div id="secretario" class="form-group col-md-6">
							<label for="secretario">Secretário</label> <input type="text"
								name="celSecretario" id="secretario"
								value="<?php echo strtoupper(@$ret[0]['celSecretario']);?>"
								onkeyup="maiuscula(this)" class="form-control">
						</div>
					</div>
					<div class="form-row">
						<div id="anfitriao" class="form-group col-md-6">
							<label for="anfitriao">Anfitrião</label> <input type="text"
								name="celAnfitriao" id="anfitriao"
								value="<?php echo strtoupper(@$ret[0]['celAnfitriao']);?>"
								onkeyup="maiuscula(this)" class="form-control">
						</div>
						<div id="colaborador" class="form-group col-md-6">
							<label for="colaborador">Colaborador</label> <input type="text"
								name="celColaborador" id="colaborador"
								value="<?php echo strtoupper(@$ret[0]['celColaborador']);?>"
								onkeyup="maiuscula(this)" class="form-control">
						</div>
					</div>
					<div class="form-row">
						<div id="dia" class="form-group col-md-2">
            				...
            				<select id="dia" name="celDia" class="form-control">
								<option value="SEGUNDA-FEIRA"
									<?=($dia == 'SEGUNDA-FEIRA')?'selected':''?>>SEGUNDA-FEIRA</option>
								<option value="TERÇA-FEIRA"
									<?=($dia == 'TERÇA-FEIRA')?'selected':''?>>TERÇA-FEIRA</option>
								<option value="QUARTA-FEIRA"
									<?=($dia == 'QUARTA-FEIRA')?'selected':''?>>QUARTA-FEIRA</option>
								<option value="QUINTA-FEIRA"
									<?=($dia == 'QUINTA-FEIRA')?'selected':''?>>QUINTA-FEIRA</option>
								<option value="SEXTA-FEIRA"
									<?=($dia == 'SEXTA-FEIRA')?'selected':''?>>SEXTA-FEIRA</option>
								<option value="SABADO" <?=($dia == 'SABADO')?'selected':''?>>SABADO</option>
								<option value="DOMINGO"
									<?=($dia == 'DOMINGO-FEIRA')?'selected':''?>>DOMINGO</option>
							</select>
						</div>
						...
						<div id="minutos" style="margin-left: -10px;"
					...
					<div class="form-row">
						<div id="endereco" class="form-group col-md-6">
							<label for="endereco">Endereço</label> <input type="text"
								name="celEndereco" id="endereco"
								value="<?php echo @$ret[0]['celEndereco'];?>"
								onkeyup="maiuscula(this)" class="form-control">
						</div>
						<div id="bairro" class="form-group col-md-6">
							<label for="bairro">Bairro</label> <input type="text"
								name="celBairro" id="bairro"
								value="<?php echo @$ret[0]['celBairro'];?>"
								onkeyup="maiuscula(this)" class="form-control">
						</div>
					</div>
					<div class="form-row">
						<div id="cidade" class="form-group col-md-6">
							<label for="cidade">Cidade</label> <input type="text"
								name="celCidade" id="cidade"
								value="<?php echo @$ret[0]['celCidade'];?>"
								onkeyup="maiuscula(this)" class="form-control">
						</div>
						<div id="estado" class="form-group col-md-6">
							<label for="estado">Estado</label> <input type="text"
								name="celEstado" id="estado"
								value="<?php echo @$ret[0]['celEstado'];?>"
								onkeyup="maiuscula(this)" class="form-control">
						</div>
					</div>
					<div class="form-row">
						<div id="pais" class="form-group col-md-6">
							<label for="pais">País</label> <input type="text" name="celPais"
								id="pais" value="<?php echo @$ret[0]['celPais'];?>"
								onkeyup="maiuscula(this)" class="form-control">
						</div>
						<div id="cep" class="form-group col-md-2">
							<label for="cep">Cep</label> <input type="text" name="celCep"
								id="cep" value="<?php echo @$ret[0]['celCep'];?>"
								onkeyup="maiuscula(this)" class="form-control">
						</div>
					</div>
					<div class="form-row">
...
<div style="margin-top: 10%;" id="modal-apaga"
	class="modal fade modal-dialog-centered" role="dialog">
		...
		<div class="modal-content">
			...
			<div class="modal-body">
				<form action="" method="get" class="form-inline">
					<a
						href="index.php?page=site_view_celulaDeleteView&id=<?php echo $id ?>&ac=delpen"
						style="background-color: red; margin: 30px auto 0px auto; width: 200px; height: 60px; font-size: 25px;"
						class="btn btn-primary btn-lg btn-block"> Apagar célula</a> <br />
				</form>
			</div>
			<div class="modal-footer">
				<p
					style="display: inline-block; float: left; color: green; font-style: inherit;">Esse
					registro ainda poderá ser recuperado</p>
